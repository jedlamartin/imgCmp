...
int main(int argc, const char** argv) {
  if (argc != 3) {
    std::cout << "Usage: " << argv[0] << " img1.jpg img2.jpg" << std::endl;
    return 1;
  }
  ...
  if (!ilLoadImage(argv[1])) {
    throw std::invalid_argument("Failed to open the first image!");
  }
  ...
  if (!ilLoadImage(argv[2])) {
    ...
    throw std::invalid_argument("Failed to open the second image!");
  }
  ...
  if (width1 != width2 || height1 != height2 || bpp1 != bpp2) {
    std::cout << "Images have different dimensions or formats!" << std::endl;
